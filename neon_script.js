...
let stocks = []
let closedTrades = []
const NEON_ENTITLEMENT_KEY = 'neon_entitled'
const FREE_SIGNAL_LIMIT = 3
let neonEntitled = false
let currentDetailSignal = null

const SIZING_CAPITAL_KEY = 'neon_sizing_capital_usd'
...
function confidenceToWinProb(score){
  ...
  // Full-range mapping: 0 => 0.00, 100 => 1.00.
...
function renderSizing(signal){
  ...
  const score = Number(signal.confidence_score)
  ...
  // Safety cap to avoid absurd position sizes.
...
function openPromoModal({ force = false } = {}){
  ...
  if(!force && !shouldShowPromo()) return
  ...
  promoTimer = setInterval(renderPromoCountdown, 1000)
}
const ACHIEVEMENTS = [
...
function getObfuscatedConfidence(sym){
  const seed = `${sym || 'X'}::neon_obf_v1`
  const r = seededRandom(seed)()
  return Math.floor(62 + r * 37) // 62..99
}

function getObfuscatedPositionValue(sym){
  const seed = `${sym || 'X'}::neon_pos_obf_v1`
  const rand = seededRandom(seed)
  const base = 8000 + rand() * 85000
  const jitter = (rand() - 0.5) * 4000
  return Math.max(2500, Math.round(base + jitter))
}
...
function renderStockCard(s, totalPosition){
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
      ...
      <div class="confidence ${isLockedConfidence ? 'is-locked' : ''}" aria-label="Confidence score">
        <div class="confidence-label">Confidence™ ${isLockedConfidence ? '<span class="locked-pill">NEON</span>' : ''}</div>
        <div class="confidence-value">${isLockedConfidence ? `<span class="blurred-num">${obfConf}/100</span>` : (formatConfidence(s.confidence_score) + '/100')}</div>
      </div>
    </div>
    <div class="meta">
      <div class="small">
  ...
  if(isLockedConfidence){
    const conf = el.querySelector('.confidence.is-locked')
    if(conf){
      conf.title = 'NEON only'
      conf.addEventListener('click', (e)=>{ e.stopPropagation(); openPromoModal({ force: true }) })
      conf.addEventListener('keydown', (e)=>{ if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); openPromoModal({ force: true }) } })
      conf.tabIndex = 0
      conf.role = 'button'
    }
  }
...
function renderLockedSignalsCard(hiddenCount){
  const el = document.createElement('div')
  el.className = 'card is-locked'
  el.tabIndex = 0
  el.role = 'button'
  el.innerHTML = `
    <div class="ticker">Unlock NEON</div>
    <div class="name">${hiddenCount} more signal${hiddenCount===1 ? '' : 's'} locked</div>
    <div class="small" style="margin-top:10px;color:var(--muted)">Upgrade to view all signals and Confidence Score™.</div>
  `
  el.addEventListener('click', ()=> openPromoModal({ force: true }))
  el.addEventListener('keydown', (e)=>{ if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openPromoModal({ force: true }) } })
  return el
}
...
function renderSignalsSidebar(list, totalPosition){
  ...
  const isLocked = !neonEntitled
  const wireLockCard = (el)=>{
    if(!el) return
    el.classList.toggle('neon-locked', isLocked)
    if(isLocked){
      el.title = 'NEON only'
      if(!el.dataset.lockWired){
        el.addEventListener('click', ()=> openPromoModal({ force: true }))
        el.addEventListener('keydown', (e)=>{ if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openPromoModal({ force: true }) } })
        el.tabIndex = 0
        el.role = 'button'
  ...
  wireLockCard(posCard)
  wireLockCard(statsCard)

  const items = (list || []).map(s => {
    const ticker = s?.ticker || ''
    const name = s?.name || ''
    const value = isLocked ? getObfuscatedPositionValue(ticker || name) : getSignalPositionValue(s)
  ...
  const sidebarTotal = isLocked ? items.reduce((sum, x)=> sum + x.value, 0) : totalPosition
  drawPositionPie(pie, slices, sidebarTotal)

  legend.innerHTML = ''
  for(const s of slices){
    const pct = sidebarTotal > 0 ? (s.value / sidebarTotal) * 100 : 0
  ...
  stats.innerHTML = `
    ...
    <div class="stat-row"><span class="k">Total</span><span class="v">${formatUsdFixed(sidebarTotal || 0)}</span></div>
...
async function openDetail(stockOrTicker, source='signals'){
  ...
  const setBlurredText = (id, v) => {
    const el = document.getElementById(id)
    if(!el) return
    el.textContent = (v==null || v==='') ? '-' : String(v)
    el.classList.add('blurred-num')
    el.setAttribute('aria-label', 'NEON locked')
    el.title = 'NEON only'
  }
  const clearBlurred = (id) => {
    const el = document.getElementById(id)
    if(!el) return
    el.classList.remove('blurred-num')
  ...
  if(allowRealConfidence){
    ...
    setBlurredText('d_confidence', `${getObfuscatedConfidence(sym)}/100`)
    renderConfidenceGauge(0)
  }
  ...
  if(sizeBtn){
    sizeBtn.disabled = !neonEntitled
    sizeBtn.title = neonEntitled ? 'Open position sizing' : 'NEON only'
    sizeBtn.onclick = (e)=>{
      e.preventDefault()
      e.stopPropagation()
      openSizingModal(s)
...
document.addEventListener('DOMContentLoaded', async ()=>{
  applySavedTheme()
  applySavedNeon()
  neonEntitled = localStorage.getItem(NEON_ENTITLEMENT_KEY) === '1'
  if(neonEntitled) document.documentElement.classList.add('neon-entitled')
  else document.documentElement.classList.remove('neon-entitled')
  if(!neonEntitled){
    try{
      const eggKey = 'neon_devtools_egg_shown'
      if(sessionStorage.getItem(eggKey) !== '1'){
        sessionStorage.setItem(eggKey, '1')
        console.warn(
          '%cHeads up:%c some metrics you see in the DOM/JS are placeholders (obfuscated) in FREE mode. Unlock NEON to see real sizing + confidence.',
          'color:#00d1ff;font-weight:900',
  ...
  const confidencePromo = document.getElementById('confidencePromo')
  const confidencePromoBtn = document.getElementById('confidencePromoBtn')
  if(confidencePromo) confidencePromo.style.display = neonEntitled ? 'none' : ''
  if(confidencePromoBtn) confidencePromoBtn.addEventListener('click', ()=> openPromoModal({ force: true }))
  if(themeToggle){
  ...
  function redraw(){
    ...
    container.innerHTML = ''
    const fullList = sortSignals(applyFilter(stocks, q, tag), sortKey)
    const hiddenCount = (!neonEntitled && fullList.length > FREE_SIGNAL_LIMIT) ? (fullList.length - FREE_SIGNAL_LIMIT) : 0
    const list = hiddenCount ? fullList.slice(0, FREE_SIGNAL_LIMIT) : fullList
    ...
    if(summaryEl){
      summaryEl.textContent = fullList.length
        ? `Total entry positions: ${formatUsdFixed(totalPosition)} (${list.length}${hiddenCount ? ` shown, ${hiddenCount} locked` : ''})`
        : ''
    }
    ...
    if(hiddenCount) container.appendChild(renderLockedSignalsCard(hiddenCount))
  }
  if(neonToggle){
